        ...
        Log.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (usrStr.length() < 3 || usrStr.length() > 20) {
                    check1 = 1;
                    mainToast = Toast.makeText(Login.this.getApplicationContext(), "Username length must be between 3-20 characters", Toast.LENGTH_SHORT);
                    mainToast.show();
                }

                if (c == y) {
                    check1 = 1;
                    mainToast = Toast.makeText(Login.this.getApplicationContext(), "Username already exist", Toast.LENGTH_SHORT);
                    mainToast.show();
                ...
                if (!(passStr.equals(passConStr))) {
                    check1 = 1;
                    mainToast = Toast.makeText(Login.this.getApplicationContext(), "Password don't match !", Toast.LENGTH_SHORT);
                    mainToast.show();
                }

                if (passStr.length() < 3 || passStr.length() > 20) {
                    check1 = 1;
                    mainToast = Toast.makeText(Login.this.getApplicationContext(), "Password length must be between 3-20 characters", Toast.LENGTH_SHORT);
                    mainToast.show();
                }
                ...
                if (ageStr.isEmpty() || nameStr.isEmpty() || heightStr.isEmpty() || weightStr.isEmpty() || passStr.isEmpty() || passConStr.isEmpty() || emailStr.isEmpty() || usrStr.isEmpty()) {
                    check1 = 1;
                    mainToast = Toast.makeText(Login.this.getApplicationContext(), "Please fill all your data ", Toast.LENGTH_SHORT);
                    mainToast.show();
                } else if (check1 == 0) {
